...
heap_t *heap_create(int (*compare)(const void *, const void *)) {
	...
	heap->size = 7;
	heap->entries = 0;
	heap->items = (void **)calloc(heap->size, sizeof(void *));
...
void heap_sift_down(heap_t *heap) {
	...
	do {
		swap_a = child_a < heap->entries && heap->items[child_a] != NULL
			  && heap->compare(heap->items[index], heap->items[child_a]) > 0;
		swap_b = child_b < heap->entries && heap->items[child_b] != NULL
...
void heap_insert(heap_t *heap, void *item) {
	...
	heap->items[(index = heap->entries)] = item;
...
void *heap_pop(heap_t *heap) {
	...
	heap->items[0] = heap->items[--heap->entries];
	heap->items[heap->entries] = NULL;
	heap_sift_down(heap);
